...
module.exports = () => {
    ...
    router.get('/me', [rateLimiterMiddleware, authorizeUser(true)], async(req, res) => {
        try {
            ...
            const userDatabase = await User.findById(req.user._id).select('username avatar rank balance xp rakeback levelBoxes stats affiliates.referrer fair anonymous ban createdAt').lean();
