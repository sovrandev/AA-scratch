            ...
            promises = [
                User.findByIdAndUpdate(user._id, {
                    $inc: {
                        ...
                        'limits.betToWithdraw': (user.limits.betToWithdraw - amountLimits) <= 0 ? -user.limits.betToWithdraw : -amountLimits,
                        'limits.betToRain': (user.limits.betToRain - amountLimits) <= 0 ? -user.limits.betToRain : -amountLimits
                    },
                    updatedAt: new Date().getTime()
                }, { new: true }).select('balance xp stats rakeback mute ban verifiedAt updatedAt').lean(),
