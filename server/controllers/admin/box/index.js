...
const adminSendBoxCreateSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        // Get box amount
        const amountBox = data.type === 'free' ? 0 : adminGetAmountBox(data, itemsDatabase);
        ...
        let boxDatabase = await Box.create({
            ...
            slug: slug,
            amount: amountBox,
            levelMin: data.type === 'free' ? Math.max(1, parseInt(data.levelMin) || 0) : 0,
            items: data.items,
...
const adminSendBoxRemoveSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        // Validate if the box is in database
