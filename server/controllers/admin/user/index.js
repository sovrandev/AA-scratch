...
const adminGetUserDataSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        const userDatabase = await User.findById(data.userId).select('local.email local.emailVerified roblox.id discord.id username avatar balance rakeback levelBoxes xp vault rank stats leaderboard limits ips mute ban updatedAt createdAt').lean();
    ...
    try {
        ...
        await logAdminAction(
            ...
            { 
                ...
                value: data.value,
                previous: userDatabase[data.setting.split('.')[0]]
            },
            identifier
        );
...
const adminSendUserBalanceSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        const previousBalance = userDatabase.balance;
