...
const adminGetCashierListSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        const dataDatabase = await Promise.all([
            ...
            CryptoTransaction.find({ type: 'withdraw', state: 'pending' }).sort({ createdAt: 1 }).limit(data.page * 12).select('amount data type user state createdAt').populate({ 
                path: 'user', 
                select: 'username avatar rank' 
            }).lean()
        ]);
...
const adminSendCashierCryptoActionSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        const transactionDatabase = await CryptoTransaction.findById(data.transactionId).select('amount data type user state createdAt').lean();
            ...
            if(offerDatabase.type === 'withdraw') {
                promises.push(
                    User.findByIdAndUpdate(offerDatabase.user, {
                        $inc: {
                            balance: (offerDatabase.amount - offerDatabase.amountProcessed),
                            'stats.withdraw': -(offerDatabase.amount - offerDatabase.amountProcessed)
                        },
