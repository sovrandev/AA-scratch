...
const cashierLimitedCheckSteamItems = async() => {
    try {
        // Get Steam CS:GO items data from Steam API
        const response = await axios.get(`https://api.steamapis.com/market/items/730?api_key=${process.env.STEAM_API_KEY}`);

        const steamItems = response.data.data || [];

        // Format Steam CS:GO items
        const formattedSteamItems = steamItems
            .map(item => ({
                assetId: `steam_${item.market_hash_name}`,
                name: item.market_name,
                image: item.image,
                amount: item.prices.safe,
                amountFixed: item.prices.safe,
                accepted: true,
                type: 'csgo'
            }));

        // Get Steam items from the database
        const steamItemsDatabase = await LimitedItem.find({ assetId: { $regex: '^steam_' } }).select('assetId').lean();

        // Save Steam items to database
        for(const item of formattedSteamItems) {
            const indexItem = steamItemsDatabase.findIndex((element) => element.assetId === item.assetId);

            if(indexItem === -1) {
                // Create Steam item in database
                await LimitedItem.create({ 
                    assetId: item.assetId, 
                    name: item.name,
                    image: item.image,
        ...
        console.log(`Updated ${formattedSteamItems.length} CS:GO items from Steam`);
        setTimeout(() => { cashierLimitedCheckSteamItems(); }, 1000 * 60 * 60 * 12); // Check twice a day
    } catch(err) {
        console.error('Error fetching Steam CS:GO items:', err);
        setTimeout(() => { cashierLimitedCheckSteamItems(); }, 1000 * 60 * 60 * 12);
    }
}
