...
} = require('../../../controllers/admin/streamers');

module.exports = (io, socket) => {

    socket.on('getStreamers', async(data, callback) => {
        ...
        try {
            ...
            try {
                ...
                adminGetStreamersSocket(io, socket, user, data, callback);
            } catch(err) {
    ...
    socket.on('getStreamerStats', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        try {
            const identifier = socket.handshake.headers['cf-connecting-ip'] || socket.conn.remoteAddress;
            await rateLimiter.consume(identifier);
            try {
                ...
                socketCheckUserData(user, true);
                socketCheckUserRank(user, ['admin']);
                adminGetStreamerStatsSocket(io, socket, user, data, callback);
            } catch(err) {
                callback({ success: false, error: { type: 'error', message: err.message } });
            }
        } catch(err) {
    ...
    socket.on('updateStreamerStatus', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                try {
                    ...
                    socketCheckUserData(user, true);
                    socketCheckUserRank(user, ['admin']);
                    adminUpdateStreamerStatusSocket(io, socket, user, data, callback);
                } catch(err) {
