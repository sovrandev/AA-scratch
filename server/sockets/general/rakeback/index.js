...
module.exports = (io, socket) => {
    ...
    socket.on('getRakebackData', async(data, callback) => {
        ...
        try {
            ...
            try {
                let user = null;
                if(socket.decoded !== undefined && socket.decoded !== null) { user = await User.findById(socket.decoded._id).select('username avatar rank xp levelBoxes mute ban').lean(); }
    ...
    socket.on('sendRakebackBoxClaim', async(data, callback) => {
        if(callback === undefined || typeof callback !== 'function') { return; }
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                const identifier = socket.handshake.headers['cf-connecting-ip'] || socket.conn.remoteAddress;
                ...
                await socketCheckAntiSpam(socket.decoded._id);
                try {
                    const user = await User.findById(socket.decoded._id).select('username avatar rank xp stats levelBoxes mute ban').lean();
                    socketCheckUserData(user, true);
                    settingCheck(user);
                    generalSendRakebackBoxClaimSocket(io, socket, user, data, callback);
                } catch(err) {
                    socketRemoveAntiSpam(socket.decoded._id);
                    callback({ success: false, error: { type: 'error', message: err.message } });
                }
