...
const adminCheckSendUserValueData = (data) => {
    if(data === undefined || data === null) {
        ...
        throw new Error('Your entered user id is invalid.');
    } else if(data.setting === undefined || typeof data.setting !== 'string' || ['avatar', 'username', 'rank', 'flags', 'stats.deposit', 'vault.amount', 'stats.withdraw', 'limits.blockRain', 'limits.blockTip', 'limits.limitTip', 'limits.blockSponsor', 'limits.blockLeaderboard', 'leaderboard.points', 'roles'].includes(data.setting) !== true) {
        throw new Error('Your entered setting is invalid.');
    } else if(data.value === undefined) {
        throw new Error('Your entered value is invalid.');
    } else if(data.setting === 'rank' && ['user', 'creator', 'mod', 'admin'].includes(data.value) !== true) {
