...
const adminFormatStats = (statsDatabase) => {
    ...
    for(let report of statsDatabase) {
        formated.total.profit = formated.total.profit + (report.stats.total.deposit - report.stats.total.withdraw);

        if(new Date(report.createdAt).getTime() > new Date(new Date().toISOString().slice(0, 10)).getTime() - (1000 * 60 * 60 * 24 * 30)) {
            formated.monthly.profit = formated.monthly.profit + (report.stats.total.deposit - report.stats.total.withdraw);

            if(new Date(report.createdAt).getTime() > new Date(new Date().toISOString().slice(0, 10)).getTime() - (1000 * 60 * 60 * 24 * 7)) {
                formated.weekly.profit = formated.weekly.profit + (report.stats.total.deposit - report.stats.total.withdraw);

                if(new Date(report.createdAt).toISOString().slice(0, 10) === new Date().toISOString().slice(0, 10)) { formated.daily.profit = (report.stats.total.deposit - report.stats.total.withdraw); }
            }
        }
    }
