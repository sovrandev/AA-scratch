...
const cashierCheckSendCreditDepositData = (data) => {
    ...
    } else if((data.amount) < (process.env.CREDIT_MIN_AMOUNT)) {
        throw new Error(`You can only deposit a min amount of $${parseFloat(process.env.CREDIT_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}.`);
    }
}
