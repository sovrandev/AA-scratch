      ...
      <div className={classes.casesSection}>
        {isActive && game.state !== "completed" && (
          <motion.div 
            ...
            animate={{ 
              transform: `translateX(${currentRound > 0 ? (currentRound - 1) * 68 : 0}px)`
            }}
        ...
        <motion.div 
          style={{ display: 'flex', flexDirection: 'row', gap: 4, zIndex: 100 }}
          animate={{
            transform: game.state === "completed" ? "" : `translateX(-${currentRound > 3 ? (currentRound - 3) * 68 : 0 }px)`
          }}
          transition={{ type: "spring", stiffness: 200, damping: 15 }} 
        >
          {boxes.map((box, index) => (
...
export default StaticBattleCard; 